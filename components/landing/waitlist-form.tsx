...
export function WaitlistForm() {
  ...
  useEffect(() => {
    if (transitionState !== "entering" || step === 3) {
      return;
    }
  ...
  const inputClassName =
    "!w-full !rounded-[var(--radius)] !border-[1.5px] !border-[var(--border-mid)] !bg-[var(--bg-card)] !px-4 !py-[13px] !text-[15px] !leading-[1.4] !text-[var(--text)] !shadow-none !outline-none placeholder:!text-[var(--text-soft)] focus-visible:!border-[var(--green)] focus-visible:!ring-3 focus-visible:!ring-[oklch(48%_0.120_148_/_0.10)]";

  const primaryButtonClassName =
    "!min-h-[46px] !rounded-[var(--radius)] !border !border-transparent !bg-[var(--green)] !px-[26px] !py-[13px] !text-[15px] !font-medium !leading-none !text-white !shadow-[0_1px_3px_oklch(22%_0.014_60_/_0.12)] hover:!-translate-y-px hover:!bg-[var(--green-hover)] hover:!shadow-[0_4px_12px_oklch(48%_0.120_148_/_0.28)] focus-visible:!ring-3 focus-visible:!ring-[oklch(48%_0.120_148_/_0.12)]";

  const ghostButtonClassName =
    "!min-h-[46px] !rounded-[var(--radius)] !border !border-transparent !bg-transparent !px-3 !py-[13px] !text-[15px] !font-medium !leading-none !text-[var(--text-mid)] !shadow-none hover:!bg-[var(--bg-alt)] hover:!text-[var(--text)] hover:!shadow-none";
  ...
  return (
    <div
      className={cn(
        ...
        step === 3
          ? "items-center justify-center border-[var(--green)] bg-[var(--bg-card)] shadow-[0_18px_50px_oklch(48%_0.120_148_/_0.14),0_4px_12px_oklch(22%_0.014_60_/_0.05)]"
          : "flex-col gap-5 border-[var(--border)] bg-[var(--bg-card)]",
      )}
    >
      {step === 3 ? (
        <>
          <div className="absolute top-1/2 left-1/2 z-0 size-8 -translate-x-1/2 -translate-y-1/2 rounded-full bg-[var(--green)] animate-[successSpread_1100ms_cubic-bezier(0.16,1,0.3,1)_forwards]" />
          <div className="pointer-events-none absolute inset-0 z-20" aria-hidden="true">
            {CONFETTI_PIECES.map(([x, y, rotate, delay, color], index) => (
              <span
                key={`${x}-${y}-${index}`}
                className="confetti-piece absolute top-1/2 left-1/2 block h-2.5 w-1.5 rounded-[2px]"
                style={
                  {
                    "--confetti-x": x,
                    "--confetti-y": y,
      ...
      {step !== 3 ? (
        <div className="flex items-start justify-between gap-5">
          <div className="flex flex-col gap-1.5">
            <div className="text-[11px] font-semibold tracking-[0.12em] text-[var(--green)] uppercase">{progressLabel}</div>
            <div className="max-w-[250px] text-[13px] leading-[1.5] text-[var(--text-soft)]">
              A few quick questions for early access.
            </div>
          </div>
            ...
            {[0, 1, 2].map((index) => (
              <span
                ...
                className={cn(
                  ...
                  index <= step ? "scale-[1.04] bg-[var(--green)]" : "bg-[var(--border-mid)]",
                )}
              />
            ))}
          </div>
      ...
      <div className={cn("relative z-10 w-full", step === 3 ? "h-full" : "h-[222px] max-[600px]:h-[230px]")}>
        <div className={cn("absolute inset-0 will-change-[opacity,transform,filter]", transitionState === "leaving" ? "pointer-events-none" : undefined, panelAnimation)}>
          ...
          {step === 1 ? (
            <form className="grid h-[222px] grid-rows-[72px_auto_46px] gap-4 max-[600px]:h-[230px]" onSubmit={handleSubmit}>
              <div className="flex min-h-[72px] flex-col gap-2">
                <div className="text-[11px] font-semibold tracking-[0.1em] text-[var(--text-soft)] uppercase">About your work</div>
          ...
          {step === 2 ? (
            <form className="grid h-[222px] grid-rows-[72px_auto_46px] gap-4 max-[600px]:h-[230px]" onSubmit={handleSubmit}>
              <div className="flex min-h-[72px] flex-col gap-2">
                <div className="text-[11px] font-semibold tracking-[0.1em] text-[var(--text-soft)] uppercase">Project friction</div>
                <div className="max-w-[330px] [font-family:var(--font-serif)] text-2xl leading-[1.15] tracking-[-0.02em] text-[var(--text)]">
          ...
          {step === 3 ? (
            <div className="flex h-full w-full flex-col items-center justify-center gap-3 text-center animate-[successContentPop_700ms_cubic-bezier(0.16,1,0.3,1)_260ms_both]">
              ...
              <div className="max-w-[330px] [font-family:var(--font-serif)] text-2xl leading-[1.15] tracking-[-0.02em] text-white">
                You’re on the list.
              </div>
              <p className="max-w-[260px] text-sm leading-[1.65] text-[oklch(96%_0.006_70_/_0.82)]">
